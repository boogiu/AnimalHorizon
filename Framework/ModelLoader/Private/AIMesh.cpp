...
HRESULT CAIMesh::Create_AnimVertex(ID3D11Device* pDevice)
{
	...
	for (size_t i = 0; i < NumBones; i++)
	{
		...
		// 오프셋 매트릭스 + BoneIndex 같이 출력 (OutputDebugStringA)
		{
			char buf[1024];

			sprintf_s(buf, "[OffsetMatrix] BoneIndex=%u Name=%s\n", BoneIndex, BoneName.c_str());
			OutputDebugStringA(buf);

			sprintf_s(buf, "[% .6f % .6f % .6f % .6f]\n", m_OffsetMatrix._11, m_OffsetMatrix._12, m_OffsetMatrix._13, m_OffsetMatrix._14);
			OutputDebugStringA(buf);
			sprintf_s(buf, "[% .6f % .6f % .6f % .6f]\n", m_OffsetMatrix._21, m_OffsetMatrix._22, m_OffsetMatrix._23, m_OffsetMatrix._24);
			OutputDebugStringA(buf);
			sprintf_s(buf, "[% .6f % .6f % .6f % .6f]\n", m_OffsetMatrix._31, m_OffsetMatrix._32, m_OffsetMatrix._33, m_OffsetMatrix._34);
			OutputDebugStringA(buf);
			sprintf_s(buf, "[% .6f % .6f % .6f % .6f]\n", m_OffsetMatrix._41, m_OffsetMatrix._42, m_OffsetMatrix._43, m_OffsetMatrix._44);
			OutputDebugStringA(buf);
